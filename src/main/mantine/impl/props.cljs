(ns mantine.impl.props
  ...
  Semantics (locked by the props-conversion decision, mnt-01kxe8gzn6bt):
  - Keys: hybrid kebab->camel (:label-position -> labelPosition); data-*/aria-*/--* exempt.
  - Values: shallow passthrough (React elements, fns, primitives survive untouched),
    EXCEPT the curated deep-convert set #{:style :styles :class-names/:classNames :vars}.
  ...
  - Escape hatch: the reserved key :& — raw JS object or CLJS map through plain
    clj->js (hyphens preserved) — merged LAST, overrides normal conversion."
...
(defn convert
  ...
  [props]
  (let [raw (get props escape-hatch-key)
        o #js {}]
    (doseq [[k v] (dissoc props escape-hatch-key)]
      (let [ks (key->str k)
        ...
        (case ck
          ...
          (gobj/set o ck v))))
    (when (some? raw)
      (gobj/extend o (if (map? raw) (clj->js raw) raw)))
    o))
